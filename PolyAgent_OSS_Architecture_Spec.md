...
**Phase 1.5 (implemented)**: Historical replay / backtesting replays stored or imported `MarketPriceSnapshot` bars through the paper simulator (`POST /api/backtests`). Live Gamma is not called during replay. Agents discover *where* the catalog is firing via `GET`/`POST /api/alphas/scan` (ranked `confidence × |score|` over the live universe or explicit `marketIds`). `POST /api/backtests/sweep` searches a catalog alpha's published parameter space (≤50 combinations; rank on OOS metrics when `split` is set). Optional `split: { mode: holdout | walk_forward }` on backtests and sweeps reports in-sample vs out-of-sample metrics (walk-forward includes per-fold windows). Optional `FeatureSource` plug-ins may attach namespaced extras on `features.event[sourceId]`; sources are env-gated and must never block harvest or ticks. `POST /api/alphas/research` composes scan + sweep (optional OOS) into a ranked promote payload so an agent can discover a candidate in one request. The `event_threshold` catalog alpha reads numeric `features.event` extras (HOLD when none). Optional `event` JSON on `PriceBar` / `MarketPriceSnapshot` lets agents replay that hypothesis on an inline or imported tape. The snapshot harvester persists FeatureSource extras when a source is enabled. Feature extraction carries the latest tape `event` onto a live mid that has none; scan/research `hasEvent=true` finds those markets. `GET /api/alphas/tapes` lists stored tapes so agents can inventory harvest/import history before researching.
